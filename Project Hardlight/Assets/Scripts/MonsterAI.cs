...
public abstract class MonsterAI : MonoBehaviour
{
    ...
    {
        anyValidTargets = (target != null && target.GetComponent<Attackable>().team == CombatInfo.Team.Hero && target.activeSelf && InMaxAgroRange(target.transform.position));
        if (!anyValidTargets)
        {
            currentTarget = null;
        }
    ...
    protected virtual List<Attackable> GetValidTargets()
    {
        ...
        Attackable[] enemyListTMP = FindObjectsOfType<Attackable>();
